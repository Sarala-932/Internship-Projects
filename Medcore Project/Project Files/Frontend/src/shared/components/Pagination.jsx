...
export default function Pagination({ meta, onPageChange }) {
  ...
  // Generate page numbers to show (simple logic: current, prev, next, first, last)
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-6 py-4 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
      <div className="text-sm text-slate-500 dark:text-slate-400">
        Showing <span className="font-semibold text-slate-900 dark:text-white">{startItem}</span> to <span className="font-semibold text-slate-900 dark:text-white">{endItem}</span> of <span className="font-semibold text-slate-900 dark:text-white">{total}</span> results
      </div>
